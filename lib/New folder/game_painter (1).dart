...
if (g.state == GameState.paused) {
  ...
  void _drawAnimatedBackground(Canvas canvas, double W, double H, double t) {
    ...
    // ── Floating planets ──
    final planets = [
      // (cx, cy, radius, color1, color2, orbitR, orbitSpeed, hasRing)
      (W*0.78, H*0.15, 22.0, const Color(0xFF7B3FBF), const Color(0xFF3A1A7A), W*0.015, 0.12, false),
      (W*0.18, H*0.32, 14.0, const Color(0xFF1A6BB5), const Color(0xFF0D3A6B), W*0.012, 0.09, false),
      (W*0.88, H*0.55, 32.0, const Color(0xFF9C5500), const Color(0xFF5C2D00), W*0.018, 0.07, true),
      (W*0.10, H*0.72, 18.0, const Color(0xFF2A8A4A), const Color(0xFF0A4A20), W*0.010, 0.14, false),
      (W*0.65, H*0.82, 11.0, const Color(0xFFAA4466), const Color(0xFF661A33), W*0.008, 0.18, false),
    ];

    for (final pl in planets) {
      final ox = sin(t * pl.$8) * pl.$7;
      final oy = cos(t * pl.$8 * 0.7) * pl.$7 * 0.5;
      final px = pl.$1 + ox;
      final py = pl.$2 + oy;
      final r  = pl.$3;
      ...
      // Ring (Saturn-like) for big planet
      if (pl.$9) {
        canvas.save();
        canvas.translate(px, py);
        canvas.scale(1.0, 0.28); // flatten to ellipse
