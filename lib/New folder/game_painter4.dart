...
if (g.state == GameState.paused) {
  ...
  void _drawAnimatedBackground(Canvas canvas, double W, double H, double t) {
    ...
    final mwDrift = sin(t * 0.04) * H * 0.015;
    ...
    final speeds  = [2.5, 7.0, 16.0, 28.0];
    ...
    for (int layer = 0; layer < 4; layer++) {
      for (int i = 0; i < counts[layer]; i++) {
        ...
        // Zoom — stars drift outward from center slowly
        final zoomT  = (t * speeds[layer] * 0.003) % 1.0;
        final fromCx = baseX - W / 2;
        final fromCy = baseY - H / 2;
        final x = W / 2 + fromCx * (1.0 + zoomT * 0.5);
        final y = H / 2 + fromCy * (1.0 + zoomT * 0.5);
